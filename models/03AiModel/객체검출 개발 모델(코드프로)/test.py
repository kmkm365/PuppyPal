...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if len(pred) == 0:
                if nl:
                    stats.append(
                        (torch.zeros(0, niou, dtype=torch.bool), torch.Tensor(), torch.Tensor(), tcls, [str(path)]))
                continue
                        ...
                        for j in (ious > iouv[0]).nonzero(as_tuple=False):
                            ...
                            if d.item() not in detected_set:
                                ...
                                if len(detected) == nl:  # all targets already located in image
                                    break
            # with open('ious.txt', 'a') as iouf:
            #     for item in iou_list:
            #         iouf.write(f"{item[0]} {item[1]} {item[2]}\n")

            # Append statistics (correct, conf, pcls, tcls)
            named_tuple = time.localtime()  # get struct_time
            time_string = time.strftime("%Y/%m/%d/, %H:%M:%S", named_tuple)
            iou_range_tf = [i.item() for i in correct.cpu()[0]]
            # print(time_string, path, TPS)
            with open('test_log.txt', 'a') as test_log:
                test_log.write(f"{time_string} {path} {iou_range_tf}\n")
            stats.append(
                (correct.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), tcls, [str(path)]))
    ...
    with open('test_log.txt', 'a') as test_log:
        s = ('%20s' + '%12s' * 6) % ('Class', 'Images',
                                     'Targets', 'P', 'R', 'mAP@.5', 'mAP@.5:.95')
        test_log.write(time_string + s + '\n')
        test_log.write(time_string + pf %
    ...
    if plots:
        ...
        if wandb and wandb.run:
            val_batches = [wandb.Image(str(f), caption=f.name)
                           for f in sorted(save_dir.glob('test*.jpg'))]
            wandb.log({"Images": wandb_images,
                       "Validation": val_batches}, commit=False)
