...
def ap_per_class(tp, conf, pred_cls, target_cls, paths, plot=False, save_dir='.', names=()):
    ...
    for ci, c in enumerate(unique_classes):
        i = pred_cls == c
        n_l = (target_cls == c).sum()  # number of labels // true_p + false_n
        ...
        if n_p == 0 or n_l == 0:
            ...
            with open('test_log.txt', 'a') as test_log:
                test_log.write(
                    f'{time_string} Class {c} / TrueP: {tpc[-1]}, FalseP: {fpc[-1]}, FalseN: {fnc}\n')
            ...
            for j in range(tp.shape[1]):
                ...
                # print(f'ap:{ap[ci, j]}, mpre:{mpre}, mrec{mrec}')
                if plot and j == 0:
